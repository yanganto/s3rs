...
- support s3 config file encryption feaution v0.3.3
...
| secret set <secret phrases>                                          | set up the secret to run time encrypt or decrpt the config  | O    | O   |
| secret set <hexal literal>                                           | set up the secret to run time encrypt or decrpt the config  | O    | O   |
| secret unset                                                         | clean up the the secret in memory                           | O    | O   |
| secret encrypt                                                       | encrypted and print current config on screeen               | O    | O   |
| secret encrypt <file name>                                           | encrypted and savce current  config to s3rs config folder   | O    | O   |
| secret show                                                          | show current unencrypt config on screeen                    | O    | O   |
